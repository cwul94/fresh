        ...
        if (orders && Array.isArray(orders) && orders.length !== 0) {
          for (const order of orders) {
            await db.query(
              ...
              [userId, order.order_name, order.order_category, order.order_price, order.order_quantity, order.order_date, order.order_img, order.order_address, order.order_detail]
            );
          }
        }
      }
